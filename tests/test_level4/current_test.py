...
class TestCurrent(util.TestCase):
    ...
    def test_current(self):
        ...
        self.assert_selector(self.MARKUP, "p:current", [], flags=util.HTML)

    def test_not_current(self):
        """Test not current."""

        self.assert_selector(self.MARKUP, "p:not(:current)", ["0"], flags=util.HTML)

    def test_current_func(self):
        """Test the functional form of current (should match nothing)."""

        self.assert_selector(self.MARKUP, ":current(p, div, a)", [], flags=util.HTML)
