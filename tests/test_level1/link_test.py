...
class TestLink(util.TestCase):
    ...
    def test_link(self):
        ...
        self.assert_selector(self.MARKUP, ":link", ["2"], flags=util.HTML)

        self.assert_selector(self.MARKUP, r":\liNk", ["2"], flags=util.HTML)

    def test_tag_and_link(self):
        """Test link and tag (all links are unvisited)."""

        self.assert_selector(self.MARKUP, "a:link", [], flags=util.XML)
