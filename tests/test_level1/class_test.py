...
class TestClass(util.TestCase):
    ...
    def test_class(self):
        ...
        self.assert_selector(self.MARKUP, ".foo", ["1"], flags=util.HTML)

    def test_type_and_class(self):
        """Test type and class."""

        self.assert_selector(self.MARKUP, "a.bar", ["2"], flags=util.HTML)

    def test_type_and_class_escaped_null(self):
        """Test type and class with an escaped null character."""

        self.assert_selector(self.MARKUP_NULL, r"a.\0 bar", ["2"], flags=util.HTML)

    def test_type_and_class_escaped_eof(self):
        """Test type and class with an escaped EOF."""

        self.assert_selector(self.MARKUP_NULL, "span.foo\\", ["1"], flags=util.HTML)
    ...
    def test_multiple_classes(self):
        ...
        self.assert_selector(markup, "a.foo.bar", ["4"], flags=util.HTML)
