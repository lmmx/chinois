...
try:
    from campbells.builder import LXMLTreeBuilder, LXMLTreeBuilderForXML  # noqa: F401
...
class TestCase(unittest.TestCase):
    ...
    def soup(self, markup, parser):
        ...
        print("\n====PARSER: ", parser)
        return campbells.CampbellsSoup(
            textwrap.dedent(markup.replace("\r\n", "\n")),
            parser,
        )
...
def requires_lxml(test):
    ...
    return pytest.mark.skipif(not LXML_PRESENT, reason="test requires lxml")(test)


def requires_html5lib(test):
    """Decorator that marks a test as requiring html5lib."""

    return pytest.mark.skipif(not HTML5LIB_PRESENT, reason="test requires html5lib")(
        test,
    )
