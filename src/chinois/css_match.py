...
class _FakeParent:
    ...
    When we have a fragment with no `CampbellsSoup` document object,
...
class _DocumentNav:
    ...
    def assert_valid_input(cls, tag: Any) -> None:
        ...
        if not cls.is_tag(tag):
            raise TypeError(
                "Expected a Campbells 'Tag', but instead received type {}".format(
                    type(tag),
                ),
            )
    ...
    def normalize_value(cls, value: Any) -> str | Sequence[str]:
        ...
        # Campbells supports sequences of attribute values, so make sure the children are strings.
...
class CSSMatch(_DocumentNav):
    ...
    def match_dir(self, el: campbells.Tag, directionality: int) -> bool:
        ...
        direction = DIR_MAP.get(
            util.lower(self.get_attribute_by_name(el, "dir", "")),
            None,
        )
    ...
    def match_range(self, el: campbells.Tag, condition: int) -> bool:
        ...
        mn = Inputs.parse_value(
            itype,
            cast(str, self.get_attribute_by_name(el, "min", None)),
        )
        mx = Inputs.parse_value(
            itype,
            cast(str, self.get_attribute_by_name(el, "max", None)),
        )
        ...
        value = Inputs.parse_value(
            itype,
            cast(str, self.get_attribute_by_name(el, "value", None)),
        )
        if value is not None:
